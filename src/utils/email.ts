...
export async function sendPasswordResetEmail(to: string, resetLink: string) {
  if (!resend) {
    console.log(`[Vorelix] (Resend non configuré) Lien de réinitialisation pour ${to} : ${resetLink}`);
    return;
  }
  ...
  await resend.emails.send({
    ...
    to,
    subject: "Réinitialisation de votre mot de passe Vorelix",
    html: `
      <p>Bonjour,</p>
      <p>Une demande de réinitialisation de mot de passe a été effectuée pour votre compte Vorelix.</p>
