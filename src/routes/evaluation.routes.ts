...
router.get(
  ...
  evaluationController.listForClass
);
router.post("/devoirs", authorize("ADMIN", "SURVEILLANT"), evaluationController.createDevoir);
router.delete("/devoirs/:id", authorize("ADMIN", "SURVEILLANT"), validateIdParam, evaluationController.removeDevoir);
...
router.delete(
  ...
  validateIdParam,
  evaluationController.removeCompositionDate
);
